...
public class DbHelper extends SQLiteOpenHelper {
    ...
    long addClass(String classname, String subjectName) {
        ...
        return sqLiteDatabase.insert(CLASS_TABLE_NAME, null, values);
    }

    Cursor getClassTable() {
        SQLiteDatabase sqLiteDatabase = this.getReadableDatabase();

        return sqLiteDatabase.rawQuery(SELECT_CLASS_TABLE, null);
    }

    int deleteClass(long cid) {

        SQLiteDatabase sqLiteDatabase = this.getReadableDatabase();
        return sqLiteDatabase.delete(CLASS_TABLE_NAME, C_ID+"=?", new String[]{String.valueOf(cid)});
    ...
    long updateClass(long cid,String classname, String subjectName) {
        ...
        return sqLiteDatabase.update(CLASS_TABLE_NAME, values,C_ID+"=?", new String[]{String.valueOf(cid)} );
    }
    //student table part
    long addStudent(long cid, int roll, String name) {
        ...
        return sqLiteDatabase.insert(STUDENT_TABLE_NAME, null, values);
    }
    ...
    int deleteStudent(long sid) {
        ...
        SQLiteDatabase sqLiteDatabase = this.getReadableDatabase();
        return sqLiteDatabase.delete(STUDENT_TABLE_NAME, S_ID+"=?", new String[]{String.valueOf(sid)});

    }
    long updateStudent(long cid,String name) {
        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(STUDENT_NAME_KEY, name);

        return sqLiteDatabase.update(STUDENT_TABLE_NAME, values,S_ID+"=?", new String[]{String.valueOf(cid)} );
    }
    ...
    long updateStatus(long sid, String date, String status){
        ...
        ContentValues values = new ContentValues();
        values.put(STATUS_KEY, status);
        String whereClaus = DATE_KEY + "='"+date+ "' AND " + S_ID+ "="+sid;
        return database.update(STATUS_TABLE_NAME, values,whereClaus,null );
    }
    ...
    String getStatus(long sid, String date){
        ...
        SQLiteDatabase database = this.getWritableDatabase();
        String whereClaus = DATE_KEY + "='"+date+ "' AND " + S_ID+ "="+sid;
        Cursor cursor = database.query(STATUS_TABLE_NAME, null, whereClaus,null,null,null, null);
